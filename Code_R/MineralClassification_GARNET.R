...
#
# -----
# Amphiboles, Apatites, Carbonates, Clay Minerals, Garnets, Feldspathoides,
...
input <- grt %>% # manipulating the minerals database and associate the answer with input object
...
grt_rf_over <- caret::train(MINERAL ~ ., # training the model
...
print(grt_rf_over) # Print on the console the summary of the model

#### Test

pred <- as_tibble(predict(grt_rf_over, test_data)) # predict the classes of the test set
...
pred1 <- as_tibble(predict(grt_rf_over, blind)) # predict the classes of the test set
...
input1 <- garnet %>% # manipulating the minerals database and associate the answer with input object
  group_by(MINERAL) %>% # grouping the instances by the mineral 'GROUP' class
...
garnet_rf_over <- caret::train(MINERAL ~ ., # training the model
                                data = train_data, # data for training
                                method = "rf", # method choosen for traininf
                                preProcess = c("scale", "center"), # Pre-process
                                trControl = ctrl, ntree = 150)#, type = 'prob') # Defines the hyperparameters

print(garnet_rf_over) # Print on the console the summary of the model

#### Test

pred <- as_tibble(predict(garnet_rf_over, test_data)) # predict the classes of the test set
...
saveRDS(garnet_rf_over, 'model_r/garnet.RDS')
...
(p.confmatrx <- ggplot(data = plotTable, 
    ...
    labs(title = 'Garnets classification - Random Forest (Test Set)', subtitle = paste0('N.Samples = 30 for each mineral, N.Trees = 150, 70-30% split, 5-fold, Accuracy: ', 100*round(accuracy_m1,digits = 2),'%')) +
...
CairoPDF(file = 'figures/R_RF_Garnet_Model', width = 9, height = 8) # Creates a PDF file and addresses a name/path
print(biplot)
