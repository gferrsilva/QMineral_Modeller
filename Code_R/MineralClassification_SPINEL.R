...
spin_rf_over <- caret::train(MINERAL ~ ., # training the model
...
print(spin_rf_over) # Print on the console the summary of the model

#### Test

pred <- as_tibble(predict(spin_rf_over, test_data)) # predict the classes of the test set
...
pred1 <- as_tibble(predict(spin_rf_over, blind)) # predict the classes of the test set
...
saveRDS(spin_rf_over, 'model_r/spin.RDS')
...
(p.confmatrx <- ggplot(data = plotTable, 
    ...
    labs(title = 'Spinels classification - Random Forest (Test Set)', subtitle = paste0('N.Samples = 30 for each mineral, N.Trees = 150, 70-30% split, 5-fold, Accuracy: ', 100*round(accuracy_m1,digits = 2),'%')) +
...
CairoPDF(file = 'figures/R_RF_Spinel_Model', width = 9, height = 8) # Creates a PDF file and addresses a name/path
print(biplot)
