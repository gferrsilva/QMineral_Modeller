...
minerals$MINERAL <- minerals$MINERAL %>%
  ...
  str_replace_all(replacement = '(AL)KALIFELDSPAR', pattern = 'ALKALIFELDSPAR')

felds <- minerals %>%
  filter(MINERAL != 'FELDSPAR',
         MINERAL != 'PLAGIOCLASE',
         MINERAL != '(AL)KALIFELDSPAR',
         MINERAL != 'ADULARIA',
         MINERAL != 'CELSIAN',
         MINERAL != 'PERTHITE',
         MINERAL != 'STRONALSITE',
...
input <- felds %>% # manipulating the minerals database and associate the answer with input object
  ...
  sample_n(50, replace = T) # sampling 300 instances of each 'GROUP', with replacement
...
felds_rf_over <- caret::train(MINERAL ~ ., # training the model
                                data = train_data, # data for training
                                method = "rf", # method choosen for traininf
                                preProcess = c("scale", "center"), # Pre-process
                                trControl = ctrl, ntree = 150)#, type = 'prob') # Defines the hyperparametersprint(felds_rf_over) # Print on the console the summary of the model

#### Test

pred <- as_tibble(predict(felds_rf_over, test_data)) # predict the classes of the test set
...
pred1 <- as_tibble(predict(felds_rf_over, blind)) # predict the classes of the test set
...
input1 <- feldspar %>% # manipulating the minerals database and associate the answer with input object
  group_by(MINERAL) %>% # grouping the instances by the mineral 'GROUP' class
...
feldspar_rf_over <- caret::train(MINERAL ~ ., # training the model
                                data = train_data, # data for training
                                method = "rf", # method choosen for traininf
                                preProcess = c("scale", "center"), # Pre-process
                                trControl = ctrl, ntree = 150)#, type = 'prob') # Defines the hyperparameters

print(feldspar_rf_over) # Print on the console the summary of the model

#### Test

pred <- as_tibble(predict(feldspar_rf_over, test_data)) # predict the classes of the test set
...
saveRDS(feldspar_rf_over, 'model_r/feldspar.RDS')
...
(p.confmatrx <- ggplot(data = plotTable, 
    ...
    labs(title = 'Feldspars classification - Random Forest (Test Set)', subtitle = paste0('N.Samples = 30 for each mineral, N.Trees = 150, 70-30% split, 5-fold, Accuracy: ', 100*round(accuracy_m1,digits = 2),'%')) +
...
CairoPDF(file = 'figures/R_RF_Feldspar_Model', width = 9, height = 8) # Creates a PDF file and addresses a name/path
print(biplot) # First Page, Test Set PCA Spatialization
