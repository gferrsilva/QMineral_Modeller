...
minerals$MINERAL <- minerals$MINERAL %>%
  replace_na('CARBONATE')

carb <- minerals %>%
  filter(MINERAL != 'CARBONATE')
...
carbonate_rf_over <- caret::train(MINERAL ~ ., # training the model
...
print(carbonate_rf_over) # Print on the console the summary of the model

#### Test

pred <- as_tibble(predict(carbonate_rf_over, test_data)) # predict the classes of the test set
...
pred1 <- as_tibble(predict(carbonate_rf_over, blind)) # predict the classes of the test set
...
input1 <- carbonate %>% # manipulating the minerals database and associate the answer with input object
  group_by(MINERAL) %>% # grouping the instances by the mineral 'GROUP' class
  sample_n(30, replace = T) # sampling 300 instances of each 'GROUP', with replacement
...
saveRDS(carbonate_rf_over, 'model_r/carbonate.RDS')
...
(p.confmatrx <- ggplot(data = plotTable, 
    ...
    labs(title = 'Carbonate classification - Random Forest (Test Set)', subtitle = paste0('N.Samples = 30 for each mineral, N.Trees = 150, 70-30% split, 5-fold, Accuracy: ', 100*round(accuracy_m1,digits = 2),'%')) +
...
CairoPDF(file = 'figures/R_RF_Carbonate_Model', width = 9, height = 8) # Creates a PDF file and addresses a name/path
print(biplot)
