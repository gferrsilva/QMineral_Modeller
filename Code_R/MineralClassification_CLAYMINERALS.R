...
#
# -----
# Amphiboles, Apatites, Carbonates, Clay Minerals, Clays, Feldspathoides,
...
minerals$MINERAL <- minerals$MINERAL %>%
  replace_na('CLAY MINERAL')

clay <- minerals %>%
  filter(MINERAL != 'CLAY MINERAL')
...
input <- clay %>% # manipulating the minerals database and associate the answer with input object
...
clay_rf_over <- caret::train(MINERAL ~ ., # training the model
                                data = train_data, # data for training
                                method = "rf", # method choosen for traininf
                                preProcess = c("scale", "center"), # Pre-process
                                trControl = ctrl, ntree = 150)#, type = 'prob') # Defines the hyperparametersprint(clay_rf_over) # Print on the console the summary of the model

#### Test

pred <- as_tibble(predict(clay_rf_over, test_data)) # predict the classes of the test set
...
pred1 <- as_tibble(predict(clay_rf_over, blind)) # predict the classes of the test set
...
input1 <- clay %>% # manipulating the minerals database and associate the answer with input object
  group_by(MINERAL) %>% # grouping the instances by the mineral 'GROUP' class
...
print(clay_rf_over) # Print on the console the summary of the model
...
saveRDS(clay_rf_over, 'model_r/clay.RDS')
...
(p.confmatrx <- ggplot(data = plotTable, 
    ...
    labs(title = 'Clays classification - Random Forest (Test Set)', subtitle = paste0('N.Samples = 30 for each mineral, N.Trees = 150, 70-30% split, 5-fold, Accuracy: ', 100*round(accuracy_m1,digits = 2),'%')) +
...
CairoPDF(file = 'figures/R_RF_Clay_Model', width = 9, height = 8) # Creates a PDF file and addresses a name/path
