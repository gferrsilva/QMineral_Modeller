...
input <- oliv %>% # manipulating the minerals database and associate the answer with input object
...
oliv_rf_over <- caret::train(MINERAL ~ ., # training the model
...
print(oliv_rf_over) # Print on the console the summary of the model

#### Test

pred <- as_tibble(predict(oliv_rf_over, test_data)) # predict the classes of the test set
...
pred1 <- as_tibble(predict(oliv_rf_over, blind)) # predict the classes of the test set
...
write.csv(olivine, 'data_input/olivine.csv')
...
input1 <- olivine %>% # manipulating the minerals database and associate the answer with input object
  group_by(MINERAL) %>% # grouping the instances by the mineral 'GROUP' class
...
olivine_rf_over <- caret::train(MINERAL ~ ., # training the model
                                data = train_data, # data for training
                                method = "rf", # method choosen for traininf
                                preProcess = c("scale", "center"), # Pre-process
                                trControl = ctrl, ntree = 150)#, type = 'prob') # Defines the hyperparameters

print(olivine_rf_over) # Print on the console the summary of the model

#### Test

pred <- as_tibble(predict(olivine_rf_over, test_data)) # predict the classes of the test set
...
saveRDS(olivine_rf_over, 'model_r/olivine.RDS')
...
(p.confmatrx <- ggplot(data = plotTable, 
    ...
    labs(title = 'Olivines classification - Random Forest (Test Set)', subtitle = paste0('N.Samples = 30 for each mineral, N.Trees = 150, 70-30% split, 5-fold, Accuracy: ', 100*round(accuracy_m1,digits = 2),'%')) +
...
CairoPDF(file = 'figures/R_RF_Olivine_Model', width = 9, height = 8) # Creates a PDF file and addresses a name/path
print(biplot)
