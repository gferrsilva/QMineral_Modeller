...
#
# -----
# Amphiboles, Apatites, Carbonates, Clay Minerals, Micas, Feldspathoides,
# Micas, Ilmenites, Micas, Olivines, Perovskites, Pyroxenes, Quartz, Sulfides,
# Titanite, Zircon
# -----
...
input <- mica %>% # manipulating the minerals database and associate the answer with input object
...
mica_rf_over <- caret::train(MINERAL ~ ., # training the model
...
print(mica_rf_over) # Print on the console the summary of the model

#### Test

pred <- as_tibble(predict(mica_rf_over, test_data)) # predict the classes of the test set
...
pred1 <- as_tibble(predict(mica_rf_over, blind)) # predict the classes of the test set
...
input1 <- mica %>% # manipulating the minerals database and associate the answer with input object
  group_by(MINERAL) %>% # grouping the instances by the mineral 'GROUP' class
...
saveRDS(mica_rf_over, 'model_r/mica.RDS')
...
(p.confmatrx <- ggplot(data = plotTable, 
    ...
    labs(title = 'Micas classification - Random Forest (Test Set)', subtitle = paste0('N.Samples = 30 for each mineral, N.Trees = 150, 70-30% split, 5-fold, Accuracy: ', 100*round(accuracy_m1,digits = 2),'%')) +
...
CairoPDF(file = 'figures/R_RF_Mica_Model', width = 9, height = 8) # Creates a PDF file and addresses a name/path
print(biplot)
