...
input1 <- amphibole %>% # manipulating the minerals database and associate the answer with input object
  group_by(MINERAL) %>% # grouping the instances by the mineral 'GROUP' class
...
train_data <- input1[as.vector(index), c(4,6:26)] # Selecting the train_data (GROUP + PCA)
test_data  <- input1[-index, 6:26]  # Selecting the test_data (PCA)
...
saveRDS(amphibole_rf_over, 'model_r/amphibole.RDS')
...
(p.testset <- ggplot(amphibole %>%
                       #group_by(MINERAL) %>%
                       #sample_frac(.2),
                       filter(MINERAL == 'HORNBLENDE' |
                              MINERAL == 'MAGNESIO-HORNBLENDE' |
                              MINERAL == 'ACTINOLITE' |
                              MINERAL == 'TREMOLITE' |
                              MINERAL == 'TSCHERMAKITE' |
                              MINERAL == 'FERRI-TSCHERMAKITE' |
                              MINERAL == 'FERRI-TSCHERMAKITIC HORNBLENDE' |
                              MINERAL == 'FE-TI-TSCHERMAKITE' |
                              MINERAL == 'GLAUCOPHANE' |
                              MINERAL == 'RIEBECKITE' |
                              MINERAL == 'HASTINGSITE' |
                              MINERAL == 'ARFVEDSONITE' |
                              MINERAL == 'ANTOPHYLLITE' |
                              MINERAL == 'PARGASITE' |
                              MINERAL == 'KAERSUTITE') %>%
                       group_by(MINERAL) %>%
                       sample_frac(.2),# Spatialization of Test set
...
(p.confmatrx <- ggplot(data = plotTable, 
    ...
    labs(title = 'Amphiboles classification - Random Forest (Test Set)', subtitle = paste0('N.Samples = 30/mineral, N.Trees = 150, 70-30% split, 5-fold, Accuracy: ', 100*round(accuracy_m1,digits = 2),'%')) +
...
CairoPDF(file = 'figures/R_RF_Amphibole_Model', width = 9, height = 8) # Creates a PDF file and addresses a name/path
print(biplot)
