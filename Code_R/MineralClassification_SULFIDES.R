...
# Import external data to enhance the model
sph <- read_csv('data_test/evandro.csv') %>%
  filter(MINERAL == 'SPHALERITE') %>%
  mutate(Pt = NULL,
         Au = NULL,
         Ag = NULL,
         id = 0,
...
input <- sulf %>% # manipulating the minerals database and associate the answer with input object
...
sulfide_rf_over <- caret::train(MINERAL ~ ., # training the model
...
print(sulfide_rf_over) # Print on the console the summary of the model
...
pred1 <- as_tibble(predict(sulfide_rf_over, blind)) # predict the classes of the test set
...
input1 <- sulfide %>% # manipulating the minerals database and associate the answer with input object
  group_by(MINERAL) %>% # grouping the instances by the mineral 'GROUP' class
  sample_n(100, replace = T) # sampling 300 instances of each 'GROUP', with replacement
...
saveRDS(sulfide_rf_over, 'model_r/sulfide.RDS')
...
(p.confmatrx <- ggplot(data = plotTable, 
    ...
    labs(title = 'Sulfides classification - Random Forest (Test Set)', subtitle = paste0('N.Samples = 30 for each mineral, N.Trees = 150, 70-30% split, 5-fold, Accuracy: ', 100*round(accuracy_m1,digits = 2),'%')) +
...
CairoPDF(file = 'figures/R_RF_Sulfide_Model', width = 9, height = 8) # Creates a PDF file and addresses a name/path
print(biplot)
